...
def parse_model(d, ch, verbose=True):
    ...
    """
    Python的抽象语法树模块，通常用于动态解析和执行Python表达式
    """
    import ast
    ...
    ''' 
        从字典中提取出来模型的基本配置，如类别数nc，激活函数act和不同模型的尺寸scales
    '''
    ...
    for i, (f, n, m, args) in enumerate(d["backbone"] + d["head"]):  # from, number, module, args
        ...
        if m in base_modules:
            ...
            if m is C2fCIB:
                legacy = False
        # elif m in {IdeNetSimpleBlock, IdeNetIAMBlock }:
        #     c1, c2 = ch[f], args[0]
        #     args = [c1, c2, *args[1:]]  # 强行把 c1 和 c2 塞进去
        elif m is MSGA:
            c1 = ch[f]
            c2 = c1
            args = [c1, c2]
